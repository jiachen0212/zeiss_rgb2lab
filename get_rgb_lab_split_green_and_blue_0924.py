...
def pipeline1(img, color_lower, color_upper, area_threshold):
    areas = find_areas(img, color_lower, color_upper, area_threshold)
    if len(areas) == 0:  # 没找到满足条件的area，返回1
...
def main(base_index_value, center, data_dir):
    dir_color = dict()
    ...
    for dir in dirs:
        if center:
            ...
            for path in paths:
                ...
                img = imread(path)
                # print("rgb均值: {}".format([np.mean(img[:,:,0]), np.mean(img[:,:,1]), np.mean(img[:,:,2])]))
            ...
            paths = glob.glob(os.path.join(data_dir, r"*.bmp").format(data_dir))
            for path in paths:
                img = imread(path)
                im_name = int(path.split('\\')[-1][:-4])
                ...
                color = pipeline1(img, color_lower, color_upper, area_threshold)
                dir_color["{}_{}".format(dir + base_index_value, im_name)] = color
...
if __name__ == '__main__':
    ...
    center = 1
    base_index_value = 14
    data_dir = r'D:\work\project\卡尔蔡司膜色缺陷\data\0924'
    blue_dir_name, green_dir_name = main(base_index_value, center, data_dir)

    # 生成所有文件夹的lab值json
    generate_y(data_dir, base_index_value)
