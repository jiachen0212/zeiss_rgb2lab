    ...
    for k, v in RGB.items():
        if k in tmp:
            if k == "17_7":
                ...
                print(k, v, LAB[k], gamma(float(v[2]) / 255) / gamma(float(v[1]) / 255))
            else:
...
if __name__ == "__main__":
    ...
    LAB = json.load(open(r'./1118_blue_lab.json', 'r'))
    RGB = json.load(open(r'./1118_blue_train_rgb.json', 'r'))

    save_params_dir = r'./params_js_blue'
