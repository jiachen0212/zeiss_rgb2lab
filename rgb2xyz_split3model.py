...
with open(r'./all_data_rgb6.json', 'w') as js_file:
    js_file.write(data)
...
def cross_val(X, Y, index, green_blue):
...
def load_data(json_x, json_y, index, green_blue):
    ...
    for k, v in json_x.items():
        ...
        if not green_blue:
            if dir_index <= 21:
                X.append([float(a) for a in json_x[k][:3]])
                # print(json_x[k][:3])
...
if __name__ == "__main__":
    # all data
    js_x = json.load(open(r'./all_data_rgb6.json', 'r'))
    ...
    for i in range(3):
        ...
        X, Y = load_data(js_x, js_y, i, green_blue)
        assert X.shape[0] == Y.shape[0]

        # use xgboost
        # hyperparameter_searching(X, Y, i, green_blue)
        cross_val(X, Y, i, green_blue)
